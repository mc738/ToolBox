...
module Http =
    ...
    let postJsonAsync<'T> (url: string) (client: HttpClient) (data: 'T) = task {
        try
            ...
            return
                ...
                | true ->
                    ...
                    Success body
                | false ->
                    { StatusCode = response.StatusCode.ToString(); Message = body }
                    |> RequestResult.Failure 
            
        with
        | exn -> return ({ Message = exn.Message; Exception = Some exn } |> RequestResult.Exception)
        
    }
    
    let postAndReplyJsonAsync<'T, 'R> (url: string) (client: HttpClient) (data: 'T) = task {
        ...
        | RequestResult.Success json ->
            ...
            with
            | exn -> return { Message = exn.Message; Exception = Some exn } |> RequestResult.DeserializationError
